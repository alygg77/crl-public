...
import {email, password, phone, username} from "@/app/validate/index.js";
...
function validate(){
  errors.value = {
    email: email(LoginModel.value.email) !== true ? email(LoginModel.value.email) : '',
    password: password(LoginModel.value.password) !== true ? password(LoginModel.value.password) : '',
  }
}
...
async function login() {
  ...
  const response = await user.login(LoginModel.value);
...
<template>
  ...
  <main class="flex text-sm w-full  max-w-[380px] flex-col  gap-5 justify-center items-center">
    <div class="flex flex-col w-full gap-5 p-5 rounded-md border-primary">
      <h1 class="text-xl text-center">
        Log in in your account
      </h1>
      <div class="flex flex-col gap-[5px]">
        <label for="email">Email</label>
        <input v-model="LoginModel.email" type="email" id="email" class="px-3 py-[10px] border-primary  rounded-md"
               placeholder="Email">
      </div>
      ...
      <div class="flex flex-col gap-[5px]">
        ...
        <div class="flex justify-between border-primary px-3 py-[10px] items-center">
          <input v-model="LoginModel.password" :type="showPass ? 'text' : 'password'" class="flex-1"
      ...
      <div class="flex flex-col gap-[10px]">
        ...
        <transition name="fade">
          <p v-if="isError" class="text-sm text-red_1">* Incorrect login credentials. Please try again.</p>
        </transition>
        <div class="m-auto">
          <p class="text-black/50">Don't have an account? <RouterLink to="/register"
                                                             class="text-black transition-all  hover:text-black/70">Sign
            up</RouterLink></p>
        </div>
